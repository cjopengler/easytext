...
1. 对 Vocabulary 产生子类 PretrainedVocabulary，增加 embedding matrix
2. 直接使用 `torch.nn.Embedding` 作为文本 embedding
3. `torch.nn.Embedding.from_pretrained` 来导入预训练的 embedding matrix,
但是注意 `freeze` 参数，这是来决定是否要 fine-tuning 参数来用的。
4. 关于在训练时候，有两种方式方式，fine-tuning 和 freeze. 解决方案是在
