...
| 序号 | 开始时间 | 结束时间 | 需求 |
|------|--------|--------|---------|
| 24 | 2020-12-21 | - | 基于 launcher 以及多 GPU 训练，重构 ner |
...
| 19 | 2020-11-06 | 2020-12-21 | 支持单机多GPU训练|
| 18 | 2020-09-09 | 2020-09-20 | 基于MSRA开发中文Bert+CRF ner模型|
...
1. 多 GPU 训练，不使用 `torch.distributed.launch`, 以便更好的管理启动配置, 使用基本的分布式启动命令
...
3. check point, loss 做分布式处理
4. callback 增加分布式的基础 callback
5. 注意是多进程编程，如果有共同的下载等行为，使用 `torch.distributed.barrier()` 信号量来控制。
6. 关于 loss 以及 metric 分布式数据同步问题。loss 会自动同步，使用 SUM, 而 metric 需要继承 `Synchronized`,
...
5. 需要能够处理 training 和 evaluate 两个阶段, 那么, 这需要增加 component 对象，在初始化时候引入 "is_training" 参数 
